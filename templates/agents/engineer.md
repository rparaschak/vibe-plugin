...
---
<!-- vibe-template: templates/agents/engineer.md v1 | generated 2026-07-13 | edits below this marker are yours -->
...
| Research protocol | `vibe-research-protocol` skill | How to get code facts without sweeping the repo — the ladder lives in the protocol |
...
6. Reply per the `vibe-team-protocol` done-report format with the build result.
...
- In the fix loop, verify each reviewer finding against current code per `vibe-review-protocol`, then fix exactly those findings — no more, no less — and report.
