...
This skill is the **only** sanctioned way to bring the environment up and run verification. The engineer, test-engineer, qa-engineer, and reviewer agents resolve it **by name** and run the two scripts below **verbatim** — they never improvise `docker`, `make`, or test commands of their own. If a command you need is missing here, that is an andon-cord to the team-lead, not a reason to guess.
...
```
<!-- Discover: the repo-relative path test-run.sh was written to, and which layer names actually resolve to a real command (others are `:`). e.g. ./scripts/test-run.sh integration -->
