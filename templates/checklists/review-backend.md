...
The `reviewer` agent loads this as the `backend-review` checklist. Every line is a review rule — cite it as `why: convention/<rule>`. It stacks on top of the `review-generic` baseline (always loaded); put only backend-specific rules here, not the generic ones.
