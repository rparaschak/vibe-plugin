...
| 2.4 | `templates/agents/reviewer.md` (points at kernel review-protocol; mechanized-checks-first) | audit pass; cold review Accept | active | disposition done (26 rows; binary approve/findings → kernel's graded verdicts, kernel wins; missing `effort` field to add; mechanized-checks step is NEW content); authoring |
| 2.5 | `templates/agents/test-engineer.md` | audit pass; cold review Accept | not_started | disposition done (32 rows; stale team-protocol name = 3rd occurrence of that drift); authoring queued (WIP=1) |
| 2.6 | `templates/agents/qa-engineer.md` (manual-testing skill inlined; stance paragraph deleted) | audit pass; cold review Accept | not_started | disposition done (35 rows; stance deletion confirmed; **overflow risk: est. 80–95 lines vs 70 cap** — author gets scout's compression mitigations + andon option); authoring queued |
