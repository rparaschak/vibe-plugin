...
- **Verified now:** Phase 0 complete (bad6b8d). **Phase 1 COMPLETE**: all 5 leaves passing. Kernel = 4 files, 233 lines total (task-ledger 65, research-protocol 28, team-protocol 70, review-protocol 70), each cold-reviewed Accept + cross-checked lossless; templates/workspace/ = 9 files (plan-template Tasks = ledger schema). Phase 1 verify gate met: files within budget, diff-review confirmed no protocol lost.
- **Broken / unverified:** nothing in Phases 0–1. skills/ + workspace-starter/ intentionally NOT deleted (frozen until Phase 6; current commands still reference them).
- **Next best step (resume here):** Phase 2 (12 leaves). Pipeline per agent template: disposition scout (sonnet, reads v1 agents/<name>.md + relevant skills) → author (opus, disposition only) → cross-check (sonnet) → cold review (opus vs standards/agent-standard.md). Scouts may run in parallel (read-only); authoring WIP=1. Start: dispatch disposition scouts for 2.1–2.3 (architect — must DELETE its consolidation-rule copy, now single-sourced in team-protocol; critic — stance paragraph deleted; engineer), then author sequentially. 2.10–2.12 (checklists, architecture-skill, env scripts) after the 9 agents. Protocol: migration-plan.md §2 (orchestrator never authors/reviews/reads v1 sources; no fable).
...
| 2.6 | `templates/agents/qa-engineer.md` (manual-testing skill inlined; stance paragraph deleted) | audit pass; cold review Accept | active | 53 lines; overflow avoided; cross-check 0 high / 3 low (2 fixed incl. verbatim description restored; 1 ruled LEAVE — two-homes rule beats disposition); cold review **Revise 9/12** (constraint placement + reply routing) → 5 fixes dispatched; **model note: qa=sonnet vs opus siblings — kept v1 choice, user may override**; round 2 re-review next |
| 2.7 | `templates/agents/codebase-researcher.md` | audit pass; cold review Accept | active | disposition done (31 rows; description 83→≤60 words = worst overage; missing Skills section to build; heavy dup vs kernel researcher section); authoring |
| 2.8 | `templates/agents/product-manager.md` | audit pass; cold review Accept | not_started | disposition done (37 rows; spec-template dedup = main shrink; PM research-ladder cap question for author); authoring queued (WIP=1) |
| 2.9 | `templates/agents/product-designer.md` | audit pass; cold review Accept | not_started | disposition done (22 rows; missing Boundaries section to build; "no hooks/paths" rule → zero copies, lives in templates); authoring queued |
