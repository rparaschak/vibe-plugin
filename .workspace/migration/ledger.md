...
- **Broken / unverified:** (a) 2.10 needs its round-3 fixer + review — exact 3 findings in leaf evidence (LAST round before andon per max-3 rule); (b) 2.12 author was in flight at session stop, result uncaptured — check whether templates/scripts/env-up.sh.template + test-run.sh.template + templates/workspace/environment-skill.md exist; if absent re-dispatch author (course P06 self-verifying pattern, readiness checks + named failures, arg/exit-code passthrough, ≤80 lines, slots w/ discovery+example), then cold review either way.
