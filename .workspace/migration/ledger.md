...
- **Broken / unverified:** (a) 2.10 needs its round-3 fixer + review — exact 3 findings in leaf evidence (LAST round before andon per max-3 rule); (b) 2.12 files authored + live-tested but cold review not run — dispatch it (rubric: builder-usable slots / self-verification real / consumer refs resolve incl. review-protocol Pass 3 + all four agents' environment rows / minimal / correct bash).
- **Next best step (resume here):** finish 2.10 (round-3 fixer + review) + 2.12 (cold review) per leaf-evidence notes above → commit Phase 2 complete → Phase 3 (skeletons + presets; note in 2.5 evidence: test-defect fix-routing gap to resolve in preset design; command-skeleton.md is the most load-bearing new file — see plan §3 Phase 3) → Phase 4 builder commands (4.3 distill: dedicated L10/L12 course scout first, per D4) → Phase 5 = STOP, hand to user. Protocol: migration-plan.md §2 (orchestrator never authors/reviews/reads v1 sources; scouts+fixers sonnet, authors+cold reviewers opus, no fable; WIP=1 authoring).
...
| 2.12 | `templates/scripts/`: deterministic env-up + test-run script templates, self-verifying (D7c), + environment skill template that names them; agents never improvise env commands | templates exist; cold review Accept; review-protocol Pass 3 references resolve | active | authored + LIVE-TESTED: env-up.sh.template (60 lines, 4 slots; wait_port//dev/tcp + wait_http/curl poll loops, named `FAILED —` non-zero exits — dead-port → exit 1 verified, open-port → ready verified; idempotent), test-run.sh.template (50 lines, 3 layer slots; verbatim arg passthrough + underlying exit codes verified, `all` chains stop-at-first-red), environment-skill.md (43 lines, 5 slots incl. {{VERIFICATION_TRIGGERS}} + {{TROUBLESHOOTING}} keyed to env-up's named failures; name: environment = the resolve target); bash -n clean; **cold review pending — dispatch on resume** |
