...
3. **Route & draft.** Send each surviving entry to the strongest rung that fits, per **## The promotion ladder** above. Draft the concrete artifact per rung — an executable check, a D12-shaped skill/constitution rule, or a rung-4 proposal-diff artifact under `.workspace/distill/promotions/`. Keep → no draft; for a per-run entry this means it graduates into the curated holding pen — assign it the next free L-ID on the way in. A learning whose right fix is application code becomes a follow-up item, never an inline fix.
