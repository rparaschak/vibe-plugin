...
---
<!-- vibe-template: commands/build-harness v4 | generated 2026-07-20 | edits below this marker are yours -->
...
3. **Existing harness evidence** (`Glob` only, never read bodies):
   - `.claude/skills/vibe-*` or `.claude/commands/{plan,implement,spec}.md` or `.claude/commands/*/{plan,implement,spec}.md` → evidence for `.claude`
   - `.grok/skills/vibe-*` or `.grok/commands/{plan,implement,spec}.md` or `.grok/commands/*/{plan,implement,spec}.md` → evidence for `.grok`
...
- **Loop rows** (conditional — only when the user takes the cmux loop opt-in at the checklist gate; offer the opt-in only when `which cmux` resolves): three rows — `vibe-loop-protocol` → `HARNESS_ROOT/skills/vibe-loop-protocol/SKILL.md` from `templates/kernel/loop-protocol.md`; `loop-wait` → `HARNESS_ROOT/scripts/loop-wait.sh` from `templates/scripts/loop-wait.sh.template` (fills: `STATUS_LOG=.workspace/status.log`, `POLL_INTERVAL=5`); loop-spec seed → `HARNESS_ROOT/loops/<loop-slug>.md` from `presets/<preset>/loop-spec.md`, **stamp-if-absent** (the user fills its Discover slots; `/vibe:build-loop` compiles it — build-harness never stamps a loop command).
...
## Composition spec
Composition is governed by `standards/composition-standard.md` — every stamp obeys it in full (W-A..W-I, command stamps also **W-J** / `COMMAND_PREFIX` + `{{CMD_*}}`, `<named artifacts>`, Regen stamp, generated-command header). Pass the absolute path of that standard into every command-row stamp-worker brief.
...
5. **Emit the checklist, then gate.** Write `.workspace/harness/checklist.md` in the `vibe-task-ledger` schema, one row per pre-seeded item with the four fields filled and targets under the chosen `HARNESS_ROOT` (command rows use the prefix-aware path formula). Set each row's `state` from its scout: `not_started` when ABSENT; `passing` when present, unmodified, and matching the current template. **Checklist gate:** present host root + `COMMAND_PREFIX` (empty or slug) + sample invokers (prefixed e.g. `/dev:plan`, `/dev:implement`, … or unprefixed `/plan`, `/implement`, …) + checklist summary + resolved scope (roster — lint-added roles flagged / command split / domains) + the loop opt-in when cmux was detected (take it → Loop rows join the checklist; decline → they don't; a later re-run can add them) in a single `AskUserQuestion` and stop for confirmation before stamping. Skip when `$ARGUMENTS` carries `--build`, or on a zero-drift doctor re-run where every row is already `passing`.
...
7. **Doctor (every run reaches this step).** Re-audit / edit-safe upgrade (via scouts + stamp workers, not you) / idempotency / fresh-session checks per **Doctor mode** (prefix-aware command paths; flat-vs-namespaced gaps surfaced, never silent-deleted). Any fresh-session gap becomes a new `not_started` row.
8. **Finalize.** Record evidence on every flipped row → write the checklist's Handoff block (include `HARNESS_ROOT=…` and `COMMAND_PREFIX=<value or empty>`) per `vibe-task-ledger` → tear down every scout and stamp worker so no subagent is left running, per `vibe-team-protocol`. Then report: host root, command prefix + sample invokers, rows now `passing` / still `blocked` / still `not_started`, the stamped-file inventory, and the fresh-session-test result — "this repo is vibe-ready" or exactly what remains. End with the **handoff**:
